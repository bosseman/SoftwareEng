...
public class PSA10SilverPrizmRookies_Mark11 {
	...
	public void scrapCards(String[] args) throws IOException, ParseException {
		...
		FileWriter fw = new FileWriter(file, true); // true to not over ride
		*/
		...
		/*
		 * To get the number of results and to get the page number from the number of
		 * search results
		 */
		...
		/*
		 * To get all listings from all pages - because with the codes above, we can
		 * only grab 50 per page and from page 1
		 */
		while (page < numPages) {
			if (page > 1) {
			...
			for (String element : listingPrice_extract) {
				listingPriceList.add(parse(element, Locale.US)); // parse is a method to tract double from US-format
																	// currency, check it out below
			}
		...
		for (j = 0; j < tempList[0].length; j++) {
			temp = NBARosters.parallelStream().filter(s -> tempList[0][j].toLowerCase().contains(s.toLowerCase()))
					.map(s -> s).collect(Collectors.toList());
			removeThis = toBeRemoved.parallelStream()
					.filter(s -> tempList[0][j].toLowerCase().contains(s.toLowerCase())).map(s -> s)
		...
		for (int k = 0; k < finalNameList.size(); k++) {
			...
			if(cardRepo.findByName(finalNameList.get(k)) == null) {
				ce.setCardName(finalNameList.get(k));
				ce.setCardType("PSA10SilverPrizmRookies_Mark11");
			}else {
				ce = cardRepo.findByName(finalNameList.get(k));
			}
			CardPriceHistory cph = new CardPriceHistory();
			
			//Need to obtain date
			cph.setPrice(Integer.parseInt(finalPriceList.get(k)));
